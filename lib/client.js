...
function sortCatalog(entries, key, direction) {
	...
	return [...entries].sort((a, b) => {
		const compared = key === "name" ? a.fullName.localeCompare(b.fullName, void 0, {
			numeric: true,
			sensitivity: "base"
		}) : key === "stars" ? a.stars - b.stars : a.updatedAt.localeCompare(b.updatedAt);
...
function writeCatalogCache(query, plugins) {
	const now = Date.now();
	const normalized = normalizeCatalogQuery(query);
	for (const [key, entry] of catalogCache) if (now - entry.fetchedAt > CATALOG_CACHE_TTL_MS) catalogCache.delete(key);
...
function PluginMarketplaceSettingsTab({ t }) {
	...
	const [selectedRelease, setSelectedRelease] = (0, react.useState)({});
	const [releaseChoices, setReleaseChoices] = (0, react.useState)({});
	const [showReleaseChoices, setShowReleaseChoices] = (0, react.useState)({});
	...
	const load = async (search = query, preserveMessage = false, bypassCache = false) => {
		...
		try {
			...
			const catalogPromise = cached === null ? api(`/plugins?query=${encodeURIComponent(normalizedSearch)}${bypassCache ? "&refresh=1" : ""}`).then((result) => {
	...
	const loadReleaseChoices = async (repository) => {
		...
		if (action !== null) return;
		setShowReleaseChoices((current) => ({
			...current,
			[key]: !current[key]
		}));
		if (releaseChoices[key] !== void 0) return;
		setAction(`versions:${key}`);
		try {
			...
			if (!result.plugin.validBundle || result.plugin.installSource !== "release" || result.plugin.releases.length === 0) {
				setMessage({
					kind: "error",
					text: result.plugin.reason ?? t("releaseChoicesUnavailable")
				});
				setReleaseChoices((current) => ({
					...current,
					[key]: []
				}));
				return;
			...
			setReleaseChoices((current) => ({
				...current,
				[key]: result.plugin.releases
			}));
			setSelectedRelease((current) => current[key] === void 0 && result.plugin.release !== null ? {
				...current,
				[key]: result.plugin.release.tag
			} : current);
		} catch (error) {
			setMessage({
				kind: "error",
				text: error instanceof Error ? error.message : t("releaseChoicesUnavailable")
			});
		} finally {
			setAction(null);
		}
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			visiblePlugins.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("section", {
				className: PluginMarketplaceSettingsTab_module_css_default.catalog,
				"aria-label": t("tab"),
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
					className: PluginMarketplaceSettingsTab_module_css_default.rows,
					children: visiblePlugins.map((plugin) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react.Fragment, { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								}), !isInstalled(plugin) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: PluginMarketplaceSettingsTab_module_css_default.textButton,
									disabled: isWorking,
									onClick: () => void loadReleaseChoices(plugin),
									children: actionIs(`versions:${plugin.fullName}`) ? t("loadingVersions") : t("chooseVersion")
								}) : null]
							})]
						}),
						showReleaseChoices[plugin.fullName] === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", {
							className: PluginMarketplaceSettingsTab_module_css_default.versionRow,
							children: releaseChoices[plugin.fullName] === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: PluginMarketplaceSettingsTab_module_css_default.status,
								children: t("loadingVersions")
							}) : (releaseChoices[plugin.fullName]?.length ?? 0) === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: PluginMarketplaceSettingsTab_module_css_default.status,
								children: t("releaseChoicesUnavailable")
							}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								className: PluginMarketplaceSettingsTab_module_css_default.versionPicker,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("installVersion") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
									value: selectedRelease[plugin.fullName] ?? "",
									disabled: isWorking,
									onChange: (event) => setSelectedRelease((current) => ({
										...current,
										[plugin.fullName]: event.currentTarget.value
									})),
									children: (releaseChoices[plugin.fullName] ?? []).map((release) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
										value: release.tag,
